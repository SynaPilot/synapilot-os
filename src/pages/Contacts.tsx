...
import { KanbanColumnSkeleton, ContactRowSkeleton } from '@/components/skeletons';
import {
...
import { LeadScoreIndicator } from '@/components/contacts/LeadScoreCard';
...
import { PIPELINE_STAGES, PIPELINE_STAGE_LABELS, CONTACT_ROLES, CONTACT_ROLE_LABELS, ACTIVITY_TYPES, ACTIVITY_TYPE_LABELS, type PipelineStage, type ContactRole } from '@/lib/constants';
...
const quickActivitySchema = z.object({
  ...
  name: z.string().min(1, 'Titre requis').max(100),
  date: z.string(),
});
type QuickActivityFormValues = z.infer<typeof quickActivitySchema>;

// ==================== TYPES ====================
type ViewMode = 'list' | 'kanban';
type SortOption = 'score' | 'followup' | 'urgency' | 'name' | 'last_contact';
        ...
        <CardContent className="p-3 space-y-2">
          ...
          <div className="space-y-1">
            ...
            <div className="flex items-center justify-between text-[10px]">
              <span className={cn(
                urgencyScore < 50 ? 'text-blue-400' : urgencyScore < 80 ? 'text-yellow-400' : 'text-red-400'
              )}>
                {urgencyScore < 50 ? 'Froid' : urgencyScore < 80 ? 'Tiède' : 'Chaud'}
              </span>
...
function SortableContactCard({ contact, onNavigate }: { contact: Contact; onNavigate: (id: string) => void }) {
  ...
  return (
    ...
    >
      {/* On sépare la zone de drag (icône/bord) de la zone de clic (le contenu) */}
...
function ContactRow({ contact, onNavigate, index, activities, onAddActivity }: { contact: Contact; onNavigate: (id: string) => void; index: number; activities: Activity[]; onAddActivity: (contactId: string) => void }) {
  const urgencyScore = contact.urgency_score || 0;
  const isHot = urgencyScore >= 80;
  const overdueStatus = isOverdue(contact.next_followup_date);
  const todayStatus = isToday(contact.next_followup_date);
  ...
  const getUrgencyLabel = (score: number) => {
    if (score < 50) return { label: 'Froid', color: 'text-blue-400' };
    if (score < 80) return { label: 'Tiède', color: 'text-yellow-400' };
    return { label: 'Chaud', color: 'text-red-400' };
  };
        ...
        <CardContent className="p-4">
          <div className="grid grid-cols-12 gap-4 items-center">
            ...
            <div className="col-span-12 md:col-span-4 flex items-center gap-3">
              ...
              <LeadScoreIndicator contact={contact} activities={activities} />
              ...
              <TooltipProvider>
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <button 
                        onClick={(e) => { e.stopPropagation(); onAddActivity(contact.id); }}
                        className="p-2 hover:bg-white/10 rounded-lg transition-colors"
                      >
...
export default function Contacts() {
  ...
  useSensor(PointerSensor, {
    activationConstraint: {
      distance: 8, // Crucial : autorise le clic si on bouge de moins de 8px
    },
  }),
  ...
  const form = useForm<ContactFormValues>({
    ...
    defaultValues: { full_name: '', email: '', phone: '', source: '', notes: '' },
  });

  const quickActivityForm = useForm<QuickActivityFormValues>({
    resolver: zodResolver(quickActivitySchema),
    defaultValues: { type: 'appel', name: '', date: new Date().toISOString().split('T')[0] },
  });
  ...
  // Fetch all activities for lead scoring
  const { data: allActivities } = useOrgQuery<Activity[]>('activities', {
    select: '*',
  });

  // Build activities map for quick lookup
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      hot: contacts.filter(c => (c.urgency_score || 0) >= 80).length,
  ...
  const processedContacts = useMemo(() => {
    ...
    switch (filterOption) {
      case 'overdue':
        ...
        break;
      case 'hot':
        result = result.filter(c => (c.urgency_score || 0) >= 80);
        break;
      case 'unassigned':
    ...
    switch (sortOption) {
      case 'score':
        result.sort((a, b) => {
          const scoreA = calculateLeadScore(a, activitiesMap.get(a.id) || []).score;
          const scoreB = calculateLeadScore(b, activitiesMap.get(b.id) || []).score;
    ...
    return result;
  }, [contacts, searchQuery, filterOption, sortOption, activitiesMap]);
  ...
  const createMutation = useMutation({
    mutationFn: async (values: ContactFormValues) => {
      ...
      const { error } = await supabase.from('contacts').insert({
        ...
        role: values.role || null, source: values.source || null,
  ...
  const createActivityMutation = useMutation({
    mutationFn: async (values: QuickActivityFormValues) => {
      if (!organizationId) throw new Error('Organisation non trouvée');
    ...
    onSuccess: () => {
      ...
      quickActivityForm.reset({ type: 'appel', name: '', date: new Date().toISOString().split('T')[0] });
  ...
  const sortOptions: { key: SortOption; label: string }[] = [
    { key: 'score', label: 'Potentiel business' },
  ...
  return (
    <motion.div className="space-y-6 p-8" initial="initial" animate="animate" variants={pageVariants} transition={{ duration: 0.3 }}>
          ...
          {isLoading ? (
            ...
            <motion.div variants={staggerContainer} initial="initial" animate="animate" className="space-y-3">
              {processedContacts.map((contact, index) => (
                <ContactRow key={contact.id} contact={contact} onNavigate={handleNavigateToContact} index={index} activities={activitiesMap.get(contact.id) || []} onAddActivity={setActivityContactId} />
              ))}
            </motion.div>
          )}
        </div>
      ...
      <Dialog open={activityContactId !== null} onOpenChange={(open) => {
        if (!open) {
          setActivityContactId(null);
          quickActivityForm.reset({ type: 'appel', name: '', date: new Date().toISOString().split('T')[0] });
        }
      }}>
        <DialogContent className="max-w-sm backdrop-blur-xl border-white/20">
          <DialogHeader className="bg-gradient-to-r from-blue-500/10 to-purple-500/10 -mx-6 -mt-6 px-6 pt-6 pb-4 rounded-t-xl border-b border-white/10">
          ...
          <Form {...quickActivityForm}>
            <form onSubmit={quickActivityForm.handleSubmit((v) => createActivityMutation.mutate(v))} className="space-y-4 pt-4">
              <FormField control={quickActivityForm.control} name="type" render={({ field }) => (
                <FormItem>
              ...
              <FormField control={quickActivityForm.control} name="name" render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-white">Titre</FormLabel>
                  <FormControl>
                    <Input placeholder="Ex: Appel de suivi" {...field} autoFocus className="bg-white/10 border-white/20 focus:border-blue-500" />
                  </FormControl>
                  <FormMessage className="text-blue-400" />
                </FormItem>
              )} />
              <FormField control={quickActivityForm.control} name="date" render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-white">Date</FormLabel>
                  <FormControl>
                    <Input type="date" {...field} className="bg-white/10 border-white/20 focus:border-blue-500" />
                  </FormControl>
                  <FormMessage className="text-blue-400" />
                </FormItem>
              )} />
              <Button
                type="submit"
                className="w-full bg-gradient-to-r from-blue-500 to-purple-500 hover:from-blue-600 hover:to-purple-600 shadow-lg shadow-blue-500/30"
                disabled={createActivityMutation.isPending}
              >
                {createActivityMutation.isPending ? (
                  <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Création...</>
                ) : (
                  <><Plus className="mr-2 h-4 w-4" />Créer l'activité</>
                )}
              </Button>
            </form>
          </Form>
